...
---
LeanCPM is better compared to legacy CPM rather than to Lean.
...
Some superintendents want control over their plan, or simply don’t have the resources for a professional scheduler and will do things on their own. 
...
Relationships are the links between activities. In legacy CPM, there are four types of relationships: FS, SS, FF, and SF.

In LeanCPM, we refer to relationships as dependencies, and there are only two types of dependencies: series and parallel.
...
Tasks do not have constraints. A constraint in legacy CPM is a hard date, usually fixed by an outside force, whereby tasks are constrained to. The 
